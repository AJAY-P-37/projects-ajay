...
export function AddCategories() {
  ...
  const { mutateAsync: saveCategoriesMutation } = useMutation({
    mutationFn: async ({ data }: { data: any[] }) => {
      const expensesService = new ExpensesService();
      return await expensesService.saveCategories(data as IExpensesCategory[]);
    },
    onSuccess: (data: any) => {
      Toast.success(data?.message || "Saved categories");
    },
    onError: (error) => {
  ...
  const saveCategories = async (data: any[]) => {
    await saveCategoriesMutation({ data });
  };
